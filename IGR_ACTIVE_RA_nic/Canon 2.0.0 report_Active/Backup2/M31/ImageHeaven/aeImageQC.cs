            ...
            try
            {
                ...
                if (File.Exists(imgFileName))
                {
                    ...
                    if (photoCropOperation == true)
                    {
                        ...
                        pImage = new CtrlImage(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), photoName, string.Empty);
            ...
            try
            {
                ...
                if (policyLst.SelectedItem != null)
                {
                    ...
                    if (FileorFolder.CreateFolder(destPath) == true)
                    {
                        ...
                        lblinfo.Text = "Press F5 to move to the next Deed";
                    }
                    else
...
//			frmExp.Width=this.ClientRectangle.Width;
	...
	void BoxDtlsBoxMouseClick(object sender, System.Windows.Forms.MouseEventArgs e)
        ...
        public bool CheckQCStatus()
        {
            ...
            {
                //pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyLst.Items[i].ToString(), string.Empty, string.Empty);
                string sql = "select * from image_master where proj_key = '" + Convert.ToInt32(wBox.ctrlBox.ProjectCode) + "' and batch_key = '" + Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()) + "' and box_number ='" + wBox.ctrlBox.BoxNumber.ToString() + "' and policy_number = '" + policyLst.SelectedItem.ToString() +"' and status = '24'";
