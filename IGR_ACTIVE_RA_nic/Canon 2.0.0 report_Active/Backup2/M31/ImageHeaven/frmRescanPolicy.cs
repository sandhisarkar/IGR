...
namespace ImageHeaven
{
    public partial class frmRescanPolicy : Form
    {
        ...
        private void lvwExportList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CtrlPolicy ctrlPolicy = new CtrlPolicy(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(), lvwExportList.FocusedItem.Text);
            ...
            CtrlImage pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString(), lvwExportList.FocusedItem.Text, string.Empty, string.Empty);
        ...
        private void lstImageName_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            try
            {
                if (lstImageName.SelectedIndex >= 0)
                {
                    ...
                    imagename = batchPath + "\\" + wBox.ctrlBox.BoxNumber.ToString() + "\\" + lvwExportList.FocusedItem.Text + "\\" + ihConstants._SCAN_FOLDER + "\\" + imagename;
                ...
                if (lvwExportList.Items.Count > 0)
                {
                    if (result == DialogResult.Yes)
                    {
                        for (int o = 0; o < lvwExportList.Items.Count; o++)
                        {
                            ...
                            {
                                pPolicy = new CtrlPolicy(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(), lvwExportList.Items[o].Text.ToString());
                                wPolicy = new wfePolicy(sqlCon, pPolicy);
                                if (wPolicy.UpdateStatus(eSTATES.POLICY_CREATED, crd) == true)
                                {
                                    if (wPolicy.DeleteAllPage())
                                    {
                                        policyFolder = batchPath + "\\" + wBox.ctrlBox.BoxNumber.ToString() + "\\" + lvwExportList.Items[o].Text;
