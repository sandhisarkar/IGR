            ...
            if (result == DialogResult.Yes)
            {
                if (Validation() == true)
                {
                    for (int i = 0; i < grdPolicy.Rows.Count; i++)
                    {
                        ...
                        ctrlPolicy = new CtrlPolicy(Convert.ToInt32(cmbProject.SelectedValue.ToString()), Convert.ToInt32(cmbBatch.SelectedValue.ToString()), cmbBox.Text.ToString(), policyNumber.ToString());
                        policy = new wfePolicy(sqlCon, ctrlPolicy);
                        if (grdPolicy.Rows[i].Cells[3].Value.ToString() != string.Empty)
                        {
