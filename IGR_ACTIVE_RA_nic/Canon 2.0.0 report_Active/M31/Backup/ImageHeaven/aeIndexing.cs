	...
	public partial class aeIndexing : Form, StateData
	{
        ...
        private void UpdateAllPolicyStatus()
        {
            ...
            try
            {
                if (policyLst.Items.Count > 0)
                {
                    for (int i = 0; i < policyLst.Items.Count; i++)
                    {
                        ...
                        pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyLst.Items[i].ToString());
            ...
            catch (Exception ex)
            {
                ...
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + "Image name-" + photoPath + "\n");
		...
		int ImageDelete()
		{
			...
			try
			{
                ...
                {
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), originalFile, string.Empty);
                    wImage = new wfeImage(sqlCon, pImage);
...
//            pictureControl.Refresh();
        ...
        void HoldPolicyToolStripMenuItemClick(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Error while updating Deed status......" + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
		...
		void UpdateState(eSTATES prmPageSate,string prmPageName,string prmDocType,string prmIndexImageName,string prmPolicyNumber)
		{
			...
			pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()),wBox.ctrlBox.BoxNumber.ToString(),prmPolicyNumber,prmPageName,string.Empty);
		    wfeImage wImage  = new wfeImage(sqlCon, pImage);
		...
		void BoxDtlsLoaded(object sender, EventArgs e)
		{
            ...
            if (policyLst.SelectedItem != null)
            {
                ctrlPolicy = new CtrlPolicy(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString());
                policy = new wfePolicy(sqlCon, ctrlPolicy);
                ShowImage(false);
                ShowPolicyDetails();
                ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString());
	...
	private void DisplayDocTypeCount()
	{
        ...
        private bool ChangeAndMoveFile(string prmSourceFileName, string prmDocType)
		{
            ...
            catch (Exception ex)
            {
                ...
                stateLog = new MemoryStream();
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + "Image name-" + sourceFile + "Doc type-" + prmDocType + "\n");
                stateLog.Write(tmpWrite, 0, tmpWrite.Length);
                exMailLog.Log(ex, this);
                return false;
            }
