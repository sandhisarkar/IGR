...
namespace LItems
{
	...
	public class wfeImage: wItem, StateData
	{
        ...
        public DataSet GetAllExportedImagewithProj(string policy_number)
        {
            ...
            sqlStr = "select proj_key,batch_key,box_number,page_name,status,page_name,doc_type,policy_number from image_master " +
        ...
        public DataTable GetAllNameEX(string Do_code, string RO_Code, string year, string deed_year, string deed_no)
        {
            ...
            sqlStr = "select Concat('Name: ',a.initial_name,' ',a.First_name,' ',a.Last_name,'\r',a.rel_code,'/o: ',a.Relation,'\r','\r','Address: ',a.address,'\r','District: ',a.Address_district_name,' Pin: ',a.pin,'\r','State: West Bangal, Country: India') as 'Name and Address',Concat('\r','Transaction : ',trim(f.tran_maj_name),' (',trim(g.tran_name),' )','\r','Deed Registered at: ',c.ro_name,'\r','Status : ',trim(D.EC_NAME),'\r') as 'Status and Transaction',a.other_party_code,a.more from index_of_name a,district b, ro_master c,party_CODE d ,party f,tranlist_code g,deed_details h where a.District_Code = b.district_code and a.ro_code = c.ro_code  and a.party_code = d.ec_code and h.tran_maj_code = f.tran_maj_code and g.tran_maj_code = h.tran_maj_code and g.tran_min_code = h.tran_min_code and  h.District_Code = a.District_Code and h.ro_code = a.ro_code and h.book = a.book and h.deed_year = a.deed_year and h.deed_no = a.deed_no and a.district_code = '" + Do_code + "' and c.district_code = '" + Do_code + "' and h.district_code = '" + Do_code + "'and h.ro_code = '" + RO_Code + "' and h.deed_year = '" + deed_year + "' and h.deed_no = '" + deed_no + "'and a.Ro_code = '" + RO_Code + "' and c.Ro_code = '" + RO_Code + "' and a.book = '" + year + "' and a.deed_year = '" + deed_year + "' and a.deed_no = '" + deed_no + "' order by a.item_no";
            try
            {
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(dsImage);
        ...
        public DataTable GetAllPropEX(string Do_code, string RO_Code, string year, string deed_year, string deed_no)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dsImage.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        Area = Area + dsImage.Tables[0].Rows[i][5].ToString() + " Decimal ";
                    }
                    if (dsImage.Tables[0].Rows[i][6].ToString() != "0")
                    {
                        Area = Area +dsImage.Tables[0].Rows[i][6].ToString() + " Katha ";
                    }
                    if (dsImage.Tables[0].Rows[i][7].ToString() != "0")
                    {
                        Area = Area + dsImage.Tables[0].Rows[i][7].ToString() + " Chatak ";
                    }
        ...
        public DataTable GetcsvAllPropEX1(string Do_code, string RO_Code, string year, string deed_year, string deed_no)
        {
            ...
            sqlStr = "select a.District_Code,a.RO_Code,a.Book,a.Deed_year,a.Deed_no,a.Item_no,a.Property_district_code,trim(b.district_name) as district_name,a.Property_ro_code, "+
                     ...
                     " a.jl_no,a.other_plots,a.other_khatian,a.land_type,a.refjl_no,a.exception from" +
