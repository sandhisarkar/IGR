...
namespace ImageHeaven
{
	...
	/// </summary>
	public partial class aePageCount : Form
	{
		...
		public aePageCount(OdbcConnection prmCon,Credentials prmCrd)
		{
			//
		...
		void CmbBoxLeave(object sender, EventArgs e)
		{
            ...
            if ((cmbProject.SelectedValue != null) && (cmbBatch.SelectedValue != null) && (cmbBox.SelectedValue != null))
            {
                ...
                ctrlPolicy = new CtrlPolicy(Convert.ToInt32(cmbProject.SelectedValue.ToString()), Convert.ToInt32(cmbBatch.SelectedValue.ToString()), cmbBox.Text.ToString(), "0");
                policy = new wfePolicy(sqlCon, ctrlPolicy);
                ds = policy.GetPolicyList(state);

                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    if ((int)eSTATES.POLICY_INITIALIZED == Convert.ToInt32(ds.Tables[0].Rows[i]["status"].ToString()) || (int)eSTATES.POLICY_MISSING == Convert.ToInt32(ds.Tables[0].Rows[i]["status"].ToString()) || (int)eSTATES.POLICY_ON_HOLD == Convert.ToInt32(ds.Tables[0].Rows[i]["status"].ToString()))
                    {
                        dr = dt.NewRow();
                        dr["SrlNo"] = (i - cont) + 1;
                        dr["Policy"] = ds.Tables[0].Rows[i]["policy_no"].ToString();

                        ctrlPolicy = new CtrlPolicy(Convert.ToInt32(cmbProject.SelectedValue.ToString()), Convert.ToInt32(cmbBatch.SelectedValue.ToString()), cmbBox.Text.ToString(), ds.Tables[0].Rows[i]["policy_no"].ToString());
                        policy = new wfePolicy(sqlCon, ctrlPolicy);
                ...
                grdPolicy.Columns["Photo"].ReadOnly = true;
            }
		}
		
		void AePageCountLoad(object sender, EventArgs e)
		{
                ...
                if (result == DialogResult.Yes)
                {
                    if (Validation() == true)
                    {
                        for (int i = 0; i < grdPolicy.Rows.Count; i++)
                        {
                            ...
                            ctrlPolicy = new CtrlPolicy(Convert.ToInt32(cmbProject.SelectedValue.ToString()), Convert.ToInt32(cmbBatch.SelectedValue.ToString()), cmbBox.Text.ToString(), policyNumber);
                            policy = new wfePolicy(sqlCon, ctrlPolicy);
                            if (grdPolicy.Rows[i].Cells[3].Value.ToString() != string.Empty)
                            {
                                ...
                                if (grdPolicy.Rows[i].Cells[4].Value.ToString() != string.Empty)
                                {
                                    ...
                                    if (Convert.ToInt32(grdPolicy.Rows[i].Cells[4].Value.ToString()) == 0)
                                    {
                                ...
                                if (policy.SavePolicyPageCount(pageCount, photo, state, crd) == true)
                                {
                                    if (grdPolicy.Rows[i].Cells[4].Value.ToString() != string.Empty)
                                    {
                                        policy.SaveInventoryInException(crd, Convert.ToInt32(grdPolicy.Rows[i].Cells[4].Value.ToString()));
                                    }
...
//			ctrlPolicy = new CtrlPolicy(Convert.ToInt32(cmbProject.SelectedValue.ToString()),Convert.ToInt32(cmbBatch.SelectedValue.ToString()),cmbBox.Text.ToString(),policyNo);
//			policy = new wfePolicy(sqlCon, ctrlPolicy);
//			
//			
//			if(policy.GetInventoryInExcp() == ihConstants._HOLD_POLICY_EXCEPTION)
