...
namespace ImageHeaven
{
	...
	public partial class aeFQC : Form, IMessageFilter ,StateData
	{
		...
		public aeFQC(wfeBox prmBox,OdbcConnection prmCon,Credentials prmCrd)
		{
            ...
            this.Text = "Final QC" + "Connection: " + sqlCon.ConnectionString.ToString();
        ...
        private void GetIndexDetails(string deed_no)
        {
            ...
            //    if (dg == null)
            //    {
            //        if (PopupPoint.pnt != null)
            //        {
            //            dg = new frmdialog(sqlCon, crd, dsVol,PopupPoint.pnt);
            //            //dg.StartPosition = FormStartPosition.Manual;
            //            //dg.Left = PopupPoint.pnt.X;
            //            //dg.Top = PopupPoint.pnt.Y;
            //            dg.Show();
            //        }
            //        else
            //        {
		...
		void AeFQCLoad(object sender, EventArgs e)
		{
            ...
            prmButtonCopyImageTo.Visible = false;
            ...
            Label lblSearch = (Label)BoxDtls.Controls["lblSearch"];
            TextBox txtSearch = (TextBox)BoxDtls.Controls["txtSearch"];
            ...
            policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
            BoxDtls.PopulateDelList( policyLst.SelectedItem.ToString());
            GetIndexDetails(policyLst.SelectedItem.ToString());
            ///changed in version 1.0.0.1
            DisplayDockTypes();
            DisplayDocTypeCount();
            lblTotalDeedCount.Text = policyLst.Items.Count.ToString();
            ...
            lblHold.Text = policy.GetHoldDeedCount();
        ...
        private void PopulateCmbValues()
        {
            DataSet dsProperty = new DataSet();
            DataSet dsTrans = new DataSet();
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            dsProperty = policy.getPropertyType();
            dsTrans = policy.getTransactionTypeMajor();
            if (dsTrans.Tables[0].Rows.Count > 0)
            {
                cmbTransaction.DataSource = dsTrans.Tables[0];
                cmbTransaction.DisplayMember = "tran_maj_name";
                cmbTransaction.ValueMember = "tran_maj_code";
                cmbTransaction.SelectedIndex = 0;
            }
            if (dsProperty.Tables[0].Rows.Count > 0)
            {
                cmbFlatType.DataSource = dsProperty.Tables[0];
                cmbFlatType.DisplayMember = "description";
                cmbFlatType.ValueMember = "apartment_type_code";
                cmbFlatType.SelectedIndex = 0;
            }
        }
		...
		private int RescanImage()
		{
            ...
            {
                colorMode = false;
                isOk = twScan.AcquireFixed(false, colorMode,1, 0,scanDpi);
            }
        ...
        private int ImportImageFromDir()
        {
            ...
            try
            {
                ...
                if ((imageName != null) && ((imageName != string.Empty)))
                {
                    ...
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(), string.Empty, string.Empty);
                    ...
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), newImageName, string.Empty);
                    wImage = new wfeImage(sqlCon, pImage);
            ...
            catch (Exception ex)
            {
                ...
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + "Image name-" + newImageName + "\n");
        ...
        private void PopulatePolicyCombo()
        {
            ...
            pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),"0");
	                    ...
	                    for (int i = 0; i < policyLst.Items.Count; i++)
	                        {
                                ...
                                boxNumber = wBox.ctrlBox.BoxNumber.ToString().ToString();
	                            ...
	                            pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyNo);
	                            wPolicy = new wfePolicy(sqlCon, pPolicy);
                                string newPolicyPath = GetPolicyPath(policyNo); //wPolicy.GetPolicyPath();

                                pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyNo, string.Empty, string.Empty);
	                            wImage = new wfeImage(sqlCon, pImage);
	                            int imgCount = wImage.GetImageCount();
                                int pageCount = wImage.GetMaxPageCount();
                                pageCount++;
                                ...
                                if ((Convert.ToInt32(status) != (int)eSTATES.POLICY_ON_HOLD))
                                {
                                    if ((Convert.ToInt32(status) == (int)eSTATES.POLICY_FQC) || ((Convert.ToInt32(status) == (int)eSTATES.POLICY_CHECKED)) || (Convert.ToInt32(status) == (int)eSTATES.POLICY_EXPORTED))
                                    {
                                        ...
                                        {
                                            if (Directory.Exists(newPolicyPath + "\\" + ihConstants._QC_FOLDER))
                                            {
                                                File.Copy(imageName, newPolicyPath + "\\" + ihConstants._QC_FOLDER + "\\" + newImageName, true);
                                                File.Copy(imageName, newPolicyPath + "\\" + ihConstants._SCAN_FOLDER + "\\" + origImageName, true);
                                            }
                                        }
                                    }
                                    ...
                                    //crd.created_by = "ADMIN";
                                    crd.created_dttm = dbcon.GetCurrenctDTTM(1, sqlCon);
                                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyNo, origImageName, string.Empty);
                                    wImage = new wfeImage(sqlCon, pImage);
                                    //wImage.Save(crd, eSTATES.PAGE_FQC, fileSize, ihConstants._NORMAL_PAGE, imgCount,string.Empty);
        ...
        private int MoveImage()
        {
            ...
            {
                movePos = Convert.ToInt32(txtposition.Text);
            }
            ...
            try
            {
                        ...
                        for (int i = 0; i < imageLst.Items.Count; i++)
                        {
                            if (imageLst.GetSelected(i))
                            {
                                ...
                                if (pos > 0)
                                {
                                    ...
                                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), originalFileName, string.Empty);
                                    wImage = new wfeImage(sqlCon, pImage);
                                ...
                                pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text.ToString(), string.Empty, string.Empty);
                                DataSet dsPage = wPolicy.getMovePolicyPageDetails(pImage);
                                for (int x = 0; x < dsPage.Tables[0].Rows.Count; x++)
                                {
                                    Imagearr.Add(dsPage.Tables[0].Rows[x][0].ToString());
                                }
                                    wImage = new wfeImage(sqlCon, pImage);
                                int imgCount = wImage.GetImageCount();
                                imgCount++;
                                ...
                                pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text.ToString());
                                wPolicy = new wfePolicy(sqlCon, pPolicy);
                                string toPolicyPath = GetPolicyPath(cmbPolicy.Text.ToString()); //wPolicy.GetPolicyPath();
                                if (pos <= 0)
                                {
                                ...
                                if (movePos != 0)
                                {
                                    Imagearr.Insert(movePos - 1, newImageName);
                                }
                                ...
                                if (Directory.Exists(fromPolicyPath + "\\" + ihConstants._FQC_FOLDER))
                                {
                                    ...
                                    else
                                    {
                                        if (File.Exists(fromPolicyPath + "\\" + ihConstants._FQC_FOLDER + "\\" + imageName))
                                        {
                                            File.Copy(fromPolicyPath + "\\" + ihConstants._FQC_FOLDER + "\\" + imageName, toPolicyPath + "\\" + ihConstants._QC_FOLDER + "\\" + newImageName, true);
                                            File.Copy(fromPolicyPath + "\\" + ihConstants._FQC_FOLDER + "\\" + imageName, toPolicyPath + "\\" + ihConstants._SCAN_FOLDER + "\\" + toMoveOriginalFileN, true);
                                            info = new System.IO.FileInfo(toPolicyPath + "\\" + ihConstants._QC_FOLDER + "\\" + newImageName);
                                ...
                                fileSize = info.Length;
                                fileSize = fileSize / 1024;
                                pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text, toMoveOriginalFileN, string.Empty);
                                wImage = new wfeImage(sqlCon, pImage);
                                if (imagemoved == true)
                                {
                                    if (pos <= 0)
                ...
                if (imagemoved == true)
                {
                    if (movePos != 0)
                    {
                        if (wPolicy.UpdateImageSrl(Imagearr) == true)
                        {
                            MessageBox.Show(movedImgCount + " Images moved successfully.....");
                        }
                        else
                        {
                            MessageBox.Show(movedImgCount + " Images not moved.....");
                        }
                    }
                    else
                    {
            ...
            try
            {
                if (cmbPolicy.SelectedValue != null)
                {
                    if (policyLst.SelectedItem.ToString() != cmbPolicy.Text)
                    {
                        ...
                        pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text, string.Empty, string.Empty);
                        wImage = new wfeImage(sqlCon, pImage);
                        int imgCount = wImage.GetImageCount();
                        imgCount++;
                        int pageCount = wImage.GetMaxPageCount();
                        pageCount = pageCount + 1;
                        pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text);
                        wPolicy = new wfePolicy(sqlCon, pPolicy);
                        ...
                        fileSize = info.Length;
                        fileSize = fileSize / 1024;
                        pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), cmbPolicy.Text.ToString(), toMoveOriginalFileN, string.Empty);
                        wImage = new wfeImage(sqlCon, pImage);
                        if (imagemoved == true)
                        {
                            if (pos <= 0)
		...
		private int ScanImage()
		{
            ...
            if (rdoAdf.Checked == true && rdoBW.Checked == true && rdoDuplex.Checked == true)
            {
                colorMode = false;
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC, 1,scanDpi);
            }
            if (rdoAdf.Checked == true && rdoBW.Checked == true && rdosimplex.Checked == true)
            {
                colorMode = false;
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC, 0,scanDpi);
            }
            if (rdoAdf.Checked == true && rdogrey.Checked == true && rdosimplex.Checked == true)
            {
                colorMode = true;
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC, 0, scangreyDpi);
            }
            if (rdoAdf.Checked == true && rdogrey.Checked == true && rdoDuplex.Checked == true)
            {
                colorMode = true;
                //isOk = twScan.Acquire(false, colorMode, 150, 1);
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC, 1,scangreyDpi);
            }
            if (rdoFlatbed.Checked == true && rdoBW.Checked == true)
            {
                colorMode = false;
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC_FLATBED, 0, scanDpi);
            }
            if (rdoFlatbed.Checked == true && rdogrey.Checked == true)
            {
                colorMode = true;
                isOk = twScan.AcquireFixed(false, colorMode, ihConstants.MAX_NO_SCAN_FQC_FLATBED, 0, scangreyDpi);
            }
			...
			try
			{
				...
				if(policyLst.Items.Count > 0)
				{
					for(int i=0; i<policyLst.Items.Count;i++)
					{
						...
						ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey),wBox.ctrlBox.BoxNumber.ToString(),policyLst.Items[i].ToString());
					    policy = new wfePolicy(sqlCon, ctrlPolicy);
                        ...
                        {
                            if (wBatch.GetBatchStatus(wBox.ctrlBox.BatchKey) != 66)
                            {
                        ...
                        /* 
                         * Commented on 12/02/2014 for slowing down the system by subhajit bhadury
                        ...
                        {
                            pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()),wBox.ctrlBox.BoxNumber.ToString(),policyLst.Items[i].ToString(),policyLst.Items[i].ToString() + "_000_A.TIF",string.Empty);
                            wImage  = new wfeImage(sqlCon, pImage);
...
//	                File.Delete(configFile);
		...
		int ImageDelete()
		{
            ...
            try
            {
                ...
                if (pos > 0)
                {
                    ...
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), fileName.Substring(0, pos - 4) + ".TIF", string.Empty);
                }
                else
                {
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), imageLst.SelectedItem.ToString(), string.Empty);
                }
                ...
                if (pos>0)
                {
                    ...
                    {
                        //pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), Convert.ToInt32(lblCurrentPolicy.Text.ToString()), imagename, string.Empty);
                        wImage = new wfeImage(sqlCon, pImage);
            ...
            catch (Exception ex)
            {
                ...
                stateLog = new MemoryStream();
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + "Image name-" + fileName + "\n");
                stateLog.Write(tmpWrite, 0, tmpWrite.Length);
                exMailLog.Log(ex, this);
                return 1;
            }
		...
		int ImageCopy()
		{
            ...
            if (imageLst.SelectedIndex >= 0)
            {
                ...
                if (pos > 0)
                {
                    ...
                    scanImageName = changedImageName;
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), changedImageName, string.Empty);
                    wImage = new wfeImage(sqlCon, pImage);
                    changedImageName = wImage.GetIndexedImageName();
                }
            }
            ...
            {
                File.Copy(fPath, imgFileName, true);
                ShowImage(true, path);
                ChangeSize(fPath);
            }
            else
		...
		void UpdateImageSize(long prmSize)
		{
			...
			//long fileSize;
				...
				pImage = new CtrlImage(wBox.ctrlBox.ProjectCode,wBox.ctrlBox.BatchKey,wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(),imageLst.SelectedItem.ToString(),string.Empty);
...
//            pictureControl.Refresh();
        ...
        void txtNewdeedKeyUp(object sender, KeyPressEventArgs e)
        {
...
//        }
        ...
        private string GetPolicyPath()
        {
            ...
            return batchPath + "\\" + wBox.ctrlBox.BoxNumber.ToString() + "\\" + policyLst.SelectedItem.ToString();
        }
        private string GetBoxPath()
        {
            ...
            wfeBatch wBatch = new wfeBatch(sqlCon);
            string batchPath = wBatch.GetPath(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey);
            return batchPath + "\\" + wBox.ctrlBox.BoxNumber.ToString();
        }
        private string GetPolicyPath(string policyNo)
        {
            ...
            wfeBatch wBatch = new wfeBatch(sqlCon);
            string batchPath = wBatch.GetPath(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey);
            return batchPath + "\\" + wBox.ctrlBox.BoxNumber.ToString() + "\\" + policyNo;
        }
        ...
        private void PopulateListView()
		{
			...
			pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()),wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(),imageName,string.Empty);
		    wfeImage wImage  = new wfeImage(sqlCon, pImage);
		...
		void BoxDtlsNextClicked(object sender, EventArgs e)
		{
            ...
            ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString(), policyNo);
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            if (policy.GetPolicyStatus() != (int)eSTATES.POLICY_ON_HOLD)
            {
                EnableDisbleControls(true);
		...
		bool UpdateState(eSTATES prmPageSate,string prmPageName,string prmDocType,string prmIndexImageName)
		{
            ...
            try
            {
                ...
                pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), prmPageName, string.Empty);
                wfeImage wImage = new wfeImage(sqlCon, pImage);
...
//                
            ...
            catch (Exception ex)
            {
                ...
                stateLog = new MemoryStream();
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + "Policy-" + policyLst.SelectedItem.ToString() + "\n");
                stateLog.Write(tmpWrite, 0, tmpWrite.Length);
                exMailLog.Log(ex, this);
                success = false;
            }
					    ...
					    if(hasPhotoBol == true)
					    {
					    	if((changedImageName.Substring(policyLen,6) == "_000_A") && (pos <= 0))					    
					    	{
						    ...
						    else if((changedImageName.Substring(policyLen,6) == "_000_A") && (pos > 0))
								{
									...
									pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()),wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(),photoImageName,string.Empty);
									wImage  = new wfeImage(sqlCon, pImage);
									changedImageName = wImage.GetIndexedImageName();
									if(pos > 0)
				    				{
        ...
        void ShowImage(bool prmOverWrite,string pFPath)
        {
            ...
            try
            {
                ...
                if (imageLst.SelectedIndex >= 0)
                {
                    ...
                    ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString().ToString().ToString(),  policyLst.SelectedItem.ToString());
                    policy = new wfePolicy(sqlCon, ctrlPolicy);
                    if (policy.GetPolicyPhotoStatus() == ihConstants._POLICY_CONTAINS_PHOTO)
                    {
                        hasPhotoBol = true;
                    ...
                    if (hasPhotoBol == true)
                    {
                        ...
                        {
                            photoImageName = imageLst.SelectedItem.ToString().Substring(0, pos);
                            pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString().ToString(),  policyLst.SelectedItem.ToString(), photoImageName, string.Empty);
                            wImage = new wfeImage(sqlCon, pImage);
                            changedImageName = wImage.GetIndexedImageName();
                            if (pos > 0)
                            {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error while showing the image", "Image error");
                stateLog = new MemoryStream();
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString().ToString() + "Image name-" + imgFileName + "\n");
                stateLog.Write(tmpWrite, 0, tmpWrite.Length);
                exMailLog.Log(ex, this);
            }
        }
		void BoxDtlsLoaded(object sender, EventArgs e)
		{
            ...
            //}
            //PopulateListView();
            ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString().ToString(),  policyLst.SelectedItem.ToString());
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            if (policy.GetPolicyStatus() != (int)eSTATES.POLICY_ON_HOLD)
            {
                EnableDisbleControls(true);
	...
	private void ChangeSize()
	{
        ...
        void UpdateAllStatus()
        {
            ...
            pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()),wBox.ctrlBox.BoxNumber.ToString().ToString(),policyLst.SelectedItem.ToString(),string.Empty,string.Empty);
			wImage  = new wfeImage(sqlCon, pImage);
            ...
            if ((wImage.GetImageCount(eSTATES.PAGE_NOT_INDEXED) == false) && (wImage.GetImageCount(eSTATES.PAGE_ON_HOLD) == false) && (wImage.GetImageCount(eSTATES.PAGE_RESCANNED_NOT_INDEXED) == false) && (policy.GetPolicyStatus() != (int)eSTATES.POLICY_EXPORTED) && (policy.GetPolicyStatus() != (int)eSTATES.POLICY_ON_HOLD))
            {
                ...
                for (int i = 0; i < imageLst.Items.Count; i++)
                {
                    ...
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString().ToString(),  policyLst.SelectedItem.ToString(), imageName, string.Empty);
                    wImage = new wfeImage(sqlCon, pImage);
        ...
        try
        {
            if (e.KeyCode == Keys.Insert)
            {
                ...
                if (imageDelLst.Items.Count > 0)
                {
                    ...
                    policyPath = GetPolicyPath(); //policyData.policy_path;
                    //To get the index file name from del list selected file name.
                    pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), imageDelLst.SelectedItem.ToString(), string.Empty);
                    wfeImage wImage = new wfeImage(sqlCon, pImage);
                    string changedImageName = wImage.GetIndexedImageName();
	...
	void ShowAllException()
	{
			...
			pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode),Convert.ToInt32(wBox.ctrlBox.BatchKey),wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString());
	...
	private bool ChangeAndMoveFile(string prmSourceFileName,string prmDocType)
		{
                ...
                if (origDoctype != prmDocType)
                {
                    ...
                    else
                    {
                        if (prmDocType != ihConstants.SIGNATUREPAGE_FILE)
                        {
                            ...
                            pImage = new CtrlImage(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(),  policyLst.SelectedItem.ToString(), prmSourceFileName, string.Empty);
                            wfeImage wImage = new wfeImage(sqlCon, pImage);
                    ...
                    else
                    {
                            ...
                            if (File.Exists(signatureFqcFile) == false)
                            {
                                ...
                                //File.Delete(sourcePath);
                                ctrlImg = new CtrlImage(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(), policyLst.SelectedItem.ToString() + fileCount + "_B" + ".TIF", string.Empty);
                                crd.created_dttm = dbcon.GetCurrenctDTTM(1, sqlCon);
            ...
            catch (Exception ex)
            {
                ...
                stateLog = new MemoryStream();
                tmpWrite = new System.Text.ASCIIEncoding().GetBytes("Project-" + wBox.ctrlBox.ProjectCode + " ,Batch-" + wBox.ctrlBox.BatchKey + " ,Box-" + wBox.ctrlBox.BoxNumber.ToString() + " ,File name-" + indexFileName + "Doc type-" + docType + "\n");
                stateLog.Write(tmpWrite, 0, tmpWrite.Length);
                exMailLog.Log(ex, this);
                return false;
            }
        ...
        void BoxDtls_cmbChanged(object sender, EventArgs e)
        {
            ...
            CtrlBox cBox = new CtrlBox(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey,cmbBox.Text.ToString());
	...
	void CmdOkClick(object sender, System.EventArgs e)
        ...
        void CmdFetch_Click(object sender, System.EventArgs e)
        {
            int i;
            ...
            if (int.TryParse(textBox1.Text,out i))
            {
                ...
                {
                    BoxDtls.RefreshNotify(comboBox1.Text, Convert.ToInt32(textBox1.Text),state);
                    policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
		...
		void SetListboxValue(string prmIamgeName,int prmSrlNo)
		{
			...
			ctrlImg = new CtrlImage(wBox.ctrlBox.ProjectCode,wBox.ctrlBox.BatchKey,wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(),prmIamgeName,string.Empty);
        ...
        private void ShowThumbImage(string pDocType)
        {
            ...
            policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
            pImage = new CtrlImage(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString(), string.Empty, pDocType);
            wfeImage wImage = new wfeImage(sqlCon, pImage);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string policy_no = policyLst.SelectedItem.ToString();
            string district_code = policy_no.Substring(0, 2);
            string ro_code = policy_no.Substring(2, 2);
            string book = policy_no.Substring(4, 1);
            string deed_year = policy_no.Substring(policy_no.IndexOf("[") - 4, 4);
            string deed_no = policy_no.Split(new char[] { '[', ']' })[1];
            if (district_code != null && ro_code != null && book != null && deed_year != null && deed_no != null)
            {
                ...
                //Changed on 12/01/2016 by Rahul
                frmDeedsummery ds = new frmDeedsummery(sqlCon, trans, crd, igr, Mode._Edit, false);

                //frmDeedsummery ds = new frmDeedsummery(sqlCon, trans, crd, igr);
                

                ds.OnCommit +=new EventHandler<DeedEventArgs>(ds_OnCommit);
                ds.OnAbort +=new EventHandler<DeedEventArgs>(ds_OnAbort);
                ds.ShowDialog();
            }
            else
            {
                MessageBox.Show(this, "Error ....", "Deed Switchboard", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void ds_OnCommit(object sender, DeedEventArgs e)
        {
            CtrlPolicy wControl = new CtrlPolicy(wBox.ctrlBox.ProjectCode, wBox.ctrlBox.BatchKey, wBox.ctrlBox.BoxNumber, policyLst.SelectedItem.ToString());
            wfePolicy wPolicy = new wfePolicy(sqlCon,wControl);
            //trans = sqlCon.BeginTransaction();
            wPolicy.UpdateBlankDeeds(trans,e._Deed.DeedHeader.page_from, e._Deed.DeedHeader.page_to);
            trans.Commit();
            //BoxDtls.NotifyPolicyRename(policyLst.SelectedIndex);
            cmdFetch_Click_1(this, null);
            string newPolicyNo = e._Deed.DeedHeader.Deed_control.District_code + e._Deed.DeedHeader.Deed_control.RO_code + e._Deed.DeedHeader.Deed_control.Book + e._Deed.DeedHeader.Deed_control.Deed_year + "[" + e._Deed.DeedHeader.Deed_control.Deed_no + "]";
            for (int i = 0; i < policyLst.Items.Count; i++)
            {
                if (policyLst.Items[i].ToString().Contains(newPolicyNo))
                {
                    policyLst.SelectedIndex = i;
                    break;
                }
            }
            //TextBox txtPageFrom = (TextBox)BoxDtls.Controls["txtPgFrom"];
            //TextBox txtPageTo = (TextBox)BoxDtls.Controls["txtPgTo"];
            //txtPageFrom.Text = e._Deed.DeedHeader.page_from;
            //txtPageTo.Text = e._Deed.DeedHeader.page_to;
            //trans = null;
        }
        private void ds_OnAbort(object sender, DeedEventArgs e)
        {
            trans.Rollback();
            //BoxDtls.NotifyPolicyRename(policyLst.SelectedIndex);
            cmdFetch_Click_1(this, null);
        ...
        private void rdoFlatbed_CheckedChanged(object sender, EventArgs e)
        {
            if (rdoFlatbed.Checked == true)
            {
        ...
        private void cmdRenameDeed_Click(object sender, EventArgs e)
        {
            OdbcTransaction trans = null;
            TextBox vol = (TextBox)BoxDtls.Controls["txtVol"];
            try
            {
                if (txtNewdeed.Text.Trim().Length == 0)
                {
                    MessageBox.Show("Please Enter new Deed Number.....");
                    return;
                }
                if (txtOldDeedno.Text.Trim().Contains(txtNewdeed.Text.Trim().PadLeft(5,'0')))
                {
                    MessageBox.Show("Unable to rename with same deed number.....");
                    return;
                }

                wQuery pQuery = new ihwQuery(sqlCon);
                DataLayerDefs.DeedControl pdc = new DataLayerDefs.DeedControl();
                DataLayerDefs.DeedControl ndc = new DataLayerDefs.DeedControl();
                string policy_no = policyLst.SelectedItem.ToString();
                ...
                for (int i = 0; i < policyLst.Items.Count; i++)
                {
                    string policyNo = policyLst.Items[i].ToString();
                    if (policyNo.Contains(txtNewdeed.Text.Trim().PadLeft(5,'0')))
                    {
                        MessageBox.Show("You are trying to rename with duplicate deed number.....");
                        return;
                    }
                }
                ...
                if (district_code != null && ro_code != null && book != null && deed_year != null && deed_no != null)
                {
                    ...
                    pdc.Book = book;
                    pdc.Deed_year = deed_year;
                    pdc.Deed_no = txtNewdeed.Text.PadLeft(5, '0');
                    
                    ndc.District_code = district_code;
                    ndc.RO_code = ro_code;
                    ndc.Book = book;
                    ndc.Deed_year = deed_year;
                    ndc.Deed_no = deed_no;
                    
                    igr_deed igr = new igr_deed(sqlCon, trans, crd, ndc);
                    if (igr.RenameDeed(pdc, vol.Text, vol.Text) == true)
                    {
                        renameDeedNumber(policy_no, pdc, trans);
                    }

                    //frmdeedControl dc = new frmdeedControl(sqlCon, crd, pdc, Mode._Rename,txtNewdeed.Text,trans);
                    //dc.ShowDialog();

                }
                else
                {
                    MessageBox.Show(this, "Error ....", "Deed Control", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void renameDeedNumber(string policy_no,DeedControl pdc,OdbcTransaction trans)
        {
            try
            {
                CtrlPolicy pPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey.ToString()), wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString());
                wfePolicy wfep = new wfePolicy(sqlCon, ctrlPolicy);

                policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                imageLst = (ListBox)BoxDtls.Controls["lstImage"];
                string oldPolicyNo = policyLst.SelectedItem.ToString();
                string policy_path = wfep.GetPolicyPath(trans);
                string newpath = policy_path.Substring(0, policy_path.LastIndexOf("\\"));
                string newPolicy = pdc.District_code.ToString() + pdc.RO_code.ToString() + pdc.Book.ToString() + pdc.Deed_year.ToString() + "[" + txtNewdeed.Text.PadLeft(5, '0') + "]";
                newpath = newpath + "\\" + newPolicy;
                wfep.RenameDeed(policy_no, newPolicy, trans, newpath);
                trans.Commit();
                string defaultImage = Path.GetDirectoryName(Application.ExecutablePath) + "\\" + "DeedRenameImage.tif";
                //img.LoadBitmapFromFile(defaultImage);
                pictureControl.Image = null;
                //policyLst.SelectedIndex = policyLst.SelectedIndex + 1;
                if (Directory.Exists(policy_path + "\\Scan"))
                {
                    DirectoryInfo d = new DirectoryInfo(policy_path + "\\Scan");
                    FileInfo[] infos = d.GetFiles();
                    foreach (FileInfo f in infos)
                    {
                        int index = f.Name.IndexOf('_');
                        string lastPart = f.Name.Substring(index);

                        File.Move(f.FullName, Path.Combine(f.DirectoryName, newPolicy + lastPart));
                    }
                }
                if (Directory.Exists(policy_path + "\\Scan\\Deleted"))
                {
                    DirectoryInfo d = new DirectoryInfo(policy_path + "\\Scan\\Deleted");
                    FileInfo[] infos = d.GetFiles();
                    foreach (FileInfo f in infos)
                    {
                        int index = f.Name.IndexOf('_');
                ...
                if (Directory.Exists(policy_path + "\\QC"))
                {
                    DirectoryInfo d = new DirectoryInfo(policy_path + "\\QC");
                    FileInfo[] infos = d.GetFiles();
                    foreach (FileInfo f in infos)
                    {
                        int index = f.Name.IndexOf('_');
                ...
                System.IO.Directory.Move(@policy_path, @newpath);
                ////policyLst.Items.RemoveAt(policyLst.SelectedIndex);
                //policyLst.Items[policyLst.SelectedIndex]= newPolicy;
                //for (int i = 0; i < imageLst.Items.Count; i++)
                //{
                //    string newName = imageLst.Items[i].ToString().Replace(oldPolicyNo,newPolicy);
                //    imageLst.Items[i] = newName;
                //}
                BoxDtls.NotifyPolicyRename(policyLst.SelectedIndex);
                //imgFileName = imgFileName.Replace(oldPolicyNo, newPolicy);
                //img.LoadBitmapFromFile(imgFileName);
                //ChangeSize();
                //policyLst.SelectedIndex = policyLst.SelectedIndex - 1;
                //policyLst.Refresh();
        ...
        PolicyDetails _pDetails = new PolicyDetails();
        DeedDetails _Details = new DeedDetails();
        private void cmdNewDeedEntry_Click(object sender, EventArgs e)
        {
            string policy_no = policyLst.SelectedItem.ToString();
            string district_code = policy_no.Substring(0, 2);
            string ro_code = policy_no.Substring(2, 2);
            ...
            string deed_year = policy_no.Substring(policy_no.IndexOf("[") - 4, 4);
            string deed_no = policy_no.Split(new char[] { '[', ']' })[1];
            _Details.Deed_control.Book = book;
            _Details.Deed_control.Deed_year = deed_year;
            _Details.Deed_control.District_code = district_code;
            _Details.Deed_control.RO_code = ro_code;
            _Details.Deed_control.Deed_no = deed_no;
            _Details.page_from = "1";
            _Details.page_to = "100";
            _Details.scan_doc_type = "X";
            _Details.tran_maj_code = "01";
            _Details.tran_min_code = "01";
            
            _pDetails.proj_key = wBox.ctrlBox.ProjectCode.ToString();
            _pDetails.batch_key = wBox.ctrlBox.BatchKey.ToString();
            _pDetails.box_number = wBox.ctrlBox.BoxNumber;
            _pDetails.Book = book;
            _pDetails.deed_year = deed_year;
            _pDetails.Do_code = district_code;
            _pDetails.Bo_Code = ro_code;
            _pDetails.policy_number = policy_no;
            _pDetails.policy_path = GetBoxPath();
            _pDetails.created_by = crd.created_by;
            _pDetails.created_dttm = crd.created_dttm;
            TextBox vol = (TextBox)BoxDtls.Controls["txtVol"];
            _pDetails.vol = vol.Text;
            _Details.volume_no = vol.Text;
            trans = sqlCon.BeginTransaction();
            frmBlankDeedEntry blkDeed = new frmBlankDeedEntry(sqlCon,crd,_pDetails,_Details,trans);
            blkDeed.OnCommit +=new EventHandler<DeedEventArgs>(blkDeed_OnCommit);
            blkDeed.OnAbort +=new EventHandler<DeedEventArgs>(blkDeed_OnAbort);
            blkDeed.ShowDialog(this);
            lblTotalDeedCount.Text = policyLst.Items.Count.ToString();
            ctrlPolicy = new CtrlPolicy(Convert.ToInt32(wBox.ctrlBox.ProjectCode), Convert.ToInt32(wBox.ctrlBox.BatchKey), wBox.ctrlBox.BoxNumber.ToString(), policyLst.SelectedItem.ToString());
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            lblHold.Text = policy.GetHoldDeedCount();
        }
        private void blkDeed_OnAbort(object sender, DeedEventArgs e)
        {
            trans.Rollback();   
        }
        private void blkDeed_OnCommit(object sender, DeedEventArgs e)
        {
            trans.Commit();
            BoxDtls.NotifyPolicyRename(policyLst.SelectedIndex);
            policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
            _pDetails.Page_from = e._Deed.DeedHeader.page_from;
            _pDetails.Page_to = e._Deed.DeedHeader.page_to;
            string newPolicyNo = e._Deed.DeedHeader.Deed_control.District_code + e._Deed.DeedHeader.Deed_control.RO_code + e._Deed.DeedHeader.Deed_control.Book+e._Deed.DeedHeader.Deed_control.Deed_year + "[" + e._Deed.DeedHeader.Deed_control.Deed_no + "]";
            for (int i = 0; i < policyLst.Items.Count; i++)
            {
                if (policyLst.Items[i].ToString().Contains(newPolicyNo))
                ...
                //policyLst.SelectedItem = newPolicyNo;
            wfePolicy wPolicy = new wfePolicy(sqlCon);
            trans = sqlCon.BeginTransaction();
            wPolicy.UpdateDeeds(trans,e._Deed.DeedHeader.Deed_control.District_code, e._Deed.DeedHeader.Deed_control.RO_code, e._Deed.DeedHeader.Deed_control.Book, e._Deed.DeedHeader.Deed_control.Deed_year, e._Deed.DeedHeader.Deed_control.Deed_no, _pDetails);
            button1_Click(_Details, null);
        }

        private void cmbDocType_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (cmbDocType.Text == "All")
            {
                groupBox7.Enabled = true;
                cmdNewDeedEntry.Enabled = true;
            }
            else { groupBox7.Enabled = false;
            cmdNewDeedEntry.Enabled = false;
            }
            if ((cmbDocType.Text == "All") || (cmbDocType.Text == "Hold") || (cmbDocType.Text == "Property Type") || (cmbDocType.Text == "Transaction Type(Major)"))
            {
            ...
            if (cmbDocType.Text == "Proper Type")
            {
                cmbFlatType.Visible = true;
            }
            else
            {
                cmbFlatType.Visible = false;
            }
            if (cmbDocType.Text == "Transaction Type(Major)")
            {
                cmbTransaction.Visible = true;
            }
            else
            {
        ...
        private void cmdFetch_Click_1(object sender, EventArgs e)
        {
            int i;
            ...
            state[6] = NovaNet.wfe.eSTATES.POLICY_EXPORTED;
            state[7] = NovaNet.wfe.eSTATES.POLICY_QC;
            if (cmbDocType.Text == "Property Type")
            {
                BoxDtls.RefreshNotifyProperty(cmbFlatType.SelectedValue.ToString(),state);
                policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
            ...
            if (cmbDocType.Text == "Transaction Type(Major)")
            {
                BoxDtls.RefreshNotifyTransaction(cmbTransaction.SelectedValue.ToString(),state);
                policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                lblRecordCount.Text = policyLst.Items.Count.ToString();
                if (policyLst.Items.Count <= 0)
                {
            ...
            if (cmbDocType.Text == "All")
            {
                BoxDtls.RefreshNotifyAll(cmbDocType.Text,state);
                policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                lblRecordCount.Text = policyLst.Items.Count.ToString();
                if (policyLst.Items.Count <= 0)
                {
            ...
            if (cmbDocType.Text == "Hold")
            {
                BoxDtls.RefreshNotifyHold(cmbDocType.Text);
                policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                lblRecordCount.Text = policyLst.Items.Count.ToString();
                if (policyLst.Items.Count <= 0)
                {
            ...
            if (int.TryParse(textBox1.Text, out i))
            {
                if ((cmbDocType.Text != string.Empty) && (comboBox1.Text != string.Empty) && (textBox1.Text != string.Empty) && (cmbDocType.Text == "Index of Name"))
                {
                    BoxDtls.RefreshNotify(comboBox1.Text, Convert.ToInt32(textBox1.Text),state);
                    policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                    lblRecordCount.Text = policyLst.Items.Count.ToString();
                ...
                if ((cmbDocType.Text != string.Empty) && (comboBox1.Text != string.Empty) && (textBox1.Text != string.Empty) && (cmbDocType.Text == "Index of Property"))
                {
                    BoxDtls.RefreshNotifyIndex_of_property(comboBox1.Text, Convert.ToInt32(textBox1.Text),state);
                    policyLst = (ListBox)BoxDtls.Controls["lstPolicy"];
                    lblRecordCount.Text = policyLst.Items.Count.ToString();
                    if (policyLst.Items.Count <= 0)
                    {
