...
namespace wSelect
{
	...
	public partial class BoxDetails : UserControl
	{
        ...
        public void NotifyPolicyRename(int index)
        {
            lstPolicy.Items.Clear();
            ArrayList arrPolicy = new ArrayList();
            wQuery pQuery = new ihwQuery(sqlCon, crd);
            ...
            lstPolicy.SelectedIndex = index;
            DataSet dsVol = pQuery.GetDeedVolume(pBox.ctrlBox.ProjectCode.ToString(), pBox.ctrlBox.BatchKey.ToString(), pBox.ctrlBox.BoxNumber.ToString(),lstPolicy.SelectedItem.ToString());
        ...
        private void PopulateFilteredPolicyListIndex_of_name(string pFilterSign, int pCount,eSTATES[] state)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyIndex_of_name(pFilterSign, pCount,state);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
        ...
        private void PopulateFilteredPolicyListIndex_of_property(string pFilterSign, int pCount,eSTATES[] state)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyIndex_of_property(pFilterSign, pCount,state);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
        ...
        private void PopulateFilteredPolicyListHold(string pDocType)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyonHold(pDocType);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
        ...
        private void PopulateFilteredPolicyListAll(string pDocType,eSTATES[] state)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyAll(pDocType,state);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
        ...
        private void PopulateFilteredPolicyListTrans(string pDocType,eSTATES[] state)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyTrans(pDocType,state);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
        ...
        private void PopulateFilteredPolicyListProperty(string pDocType,eSTATES[] state)
        {
            lstPolicy.Items.Clear();
            wQuery pQuery = new ihwQuery(sqlCon);
            CtrlPolicy ctrlPolicy;
            ...
            ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode, pBox.ctrlBox.BatchKey, pBox.ctrlBox.BoxNumber, "0");
            policy = new wfePolicy(sqlCon, ctrlPolicy);
            pDs = policy.GetFilteredPolicyProperty(pDocType,state);
            for (int i = 0; i < pDs.Tables[0].Rows.Count; i++)
            {
                policyNo = pDs.Tables[0].Rows[i]["policy_number"].ToString();
                lstPolicy.Items.Add(policyNo);
...
//            policyState[0]=currState;
        ...
        //        if (dg == null)
        //        {
        //            dg = new frmdialog(sqlCon, crd, dsVol);
        //            dg.Show();
        //        }
        //        else
        //        {
		...
		void LstPolicySelectedIndexChanged(object sender, EventArgs e)
		{
            ...
            if (PolicyChanged != null)
            {
                ...
                dsVol = pQuery.GetDeedVolume(pBox.ctrlBox.ProjectCode.ToString(), pBox.ctrlBox.BatchKey.ToString(), pBox.ctrlBox.BoxNumber.ToString(),lstPolicy.SelectedItem.ToString());
...
//			lstImageDel.Refresh();
        ...
        public void RefreshNotify(string pFilterSign,int pCount,eSTATES[] state)
        {
            try
            {
                PopulateFilteredPolicyListIndex_of_name(pFilterSign, pCount,state);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        public void RefreshNotifyIndex_of_property(string pFilterSign, int pCount,eSTATES[] state)
        {
            try
            {
                PopulateFilteredPolicyListIndex_of_property(pFilterSign, pCount,state);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        public void RefreshNotifyHold(string pDocType)
        {
            try
            {
                PopulateFilteredPolicyListHold(pDocType);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        public void RefreshNotifyAll(string pDocType,eSTATES[] state)
        {
            try
            {
                PopulateFilteredPolicyListAll(pDocType,state);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        public void RefreshNotifyTransaction(string pDocType,eSTATES[] state)
        {
            try
            {
                PopulateFilteredPolicyListTrans(pDocType,state);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        public void RefreshNotifyProperty(string pDocType,eSTATES[] state)
        {
            try
            {
                PopulateFilteredPolicyListProperty(pDocType,state);
                if (lstPolicy.Items.Count > 0)
                {
                    lstPolicy.SelectedIndex = 0;
                    PopulateImageList(lstPolicy.SelectedItem.ToString());
        ...
        private void txtSearch_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (!string.IsNullOrEmpty(txtSearch.Text.Trim()))
                {
                    ...
                    {
                        if (lstPolicy.Items[i].ToString().Contains(txtSearch.Text.Trim()))
                        {
